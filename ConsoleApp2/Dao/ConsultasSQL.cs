...
namespace c_sahrp
{
    class ConsultasSQL
    {
        ...
        {
            conexion.Open();
            SqlCommand cmd = new SqlCommand("SELECT TOP 20 [Usuario_id] ,[Trab_id],[Descr_Usuario],[Grupo_Usuario],[Password],[Fec_password],[DiasCaducidad],[Proveedor_id],[Email],[Fec_creacion],[Operador_id],[Fec_Modi] ,[Operador_modi_id],[Autorizacion],[Estado_Trab],[Estado_DB],[Estado_SRV],[st_adm],[Anexo_Telf],[Celular] FROM [PoderosaFocus].[dbo].[ADM_Usuarios]", conexion);
            ...
            if (r.HasRows){
                while (r.Read()){
                    lista.Add(r["Operador_id"].ToString());
